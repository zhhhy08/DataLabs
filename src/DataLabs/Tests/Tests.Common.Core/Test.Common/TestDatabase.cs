namespace Microsoft.WindowsAzure.Governance.DataLabs.Tests.Common
{
    ...
    public class TestDatabase : IDatabase
    {
        ...
        public TestCacheNode TestCacheNode => TestConnectionMultiplexer.TestCacheNode;

        public TestDatabase(TestConnectionMultiplexer testConnectionMultiplexer)
        {
            TestConnectionMultiplexer = testConnectionMultiplexer;
        }

        public int Database => -1;

        public IConnectionMultiplexer Multiplexer => TestConnectionMultiplexer;
        ...
        public Task<RedisResult> ExecuteAsync(string command, params object[] args)
        {
            ...
            {
                case "LASTSAVE":
                    return Task.FromResult(RedisResult.Create(TestCacheNode.GetLastSaveTime(), ResultType.SimpleString));
                case "BGSAVE":
                    return Task.FromResult(RedisResult.Create(TestCacheNode.SaveCheckPoint(bgsave: true), ResultType.SimpleString));
                case "SAVE":
                    return Task.FromResult(RedisResult.Create(TestCacheNode.SaveCheckPoint(bgsave: false), ResultType.SimpleString));
                case "SETIFPM":
                    return Task.FromResult(RedisResult.Create(TestCacheNode.SetValueIfMatchAsync((string)args[0], (ReadOnlyMemory<byte>)args[1],
                    BitConverter.ToInt64((byte[])args[2], 0)), ResultType.SimpleString));
                case "SETWPIFPGT":
                    return Task.FromResult(RedisResult.Create(TestCacheNode.SetValueIfGreaterThanAsync((string)args[0], (ReadOnlyMemory<byte>)args[1],
                    BitConverter.ToInt64((byte[])args[2], 0)), ResultType.SimpleString));
                default:
        ...
        {
            RedisValue redisValue = TestCacheNode.GetValueAsync(key);
        ...
        public Task<RedisValue[]> StringGetAsync(RedisKey[] keys, CommandFlags flags = CommandFlags.None)
        {
            ...
            {
                values[i] = TestCacheNode.GetValueAsync(keys[i]);
            }
        ...
        {
            return Task.FromResult(TestCacheNode.SetValueAsync(key, value));
        }
        ...
        {
            return Task.FromResult(TestCacheNode.SetKeyExpireAsync(key, expiry));
        }

        public Task<bool> KeyDeleteAsync(RedisKey key, CommandFlags flags = CommandFlags.None)
        {
            return Task.FromResult(TestCacheNode.DeleteKeyAsync(key));
        }
