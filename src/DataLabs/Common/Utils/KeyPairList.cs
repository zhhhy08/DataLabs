namespace Microsoft.WindowsAzure.Governance.DataLabs.Common.Core.Utils
{
    ...
    // This struct is same as .net System.Diagnostics.TagList
    // We modify the struct to be used for KeyPairList. string,string
    // This can be used for several places where we need to maintain list of keypairs
    ...
    public struct KeyPairList<TKey,TValue> : IList<KeyValuePair<TKey, TValue>>, IReadOnlyList<KeyValuePair<TKey, TValue>> 
        where TKey : notnull, IEquatable<TKey> 
        where TValue : IEquatable<TValue>
    {
        internal KeyValuePair<TKey, TValue> Tag1;
        internal KeyValuePair<TKey, TValue> Tag2;
        internal KeyValuePair<TKey, TValue> Tag3;
        internal KeyValuePair<TKey, TValue> Tag4;
        internal KeyValuePair<TKey, TValue> Tag5;
        internal KeyValuePair<TKey, TValue> Tag6;
        internal KeyValuePair<TKey, TValue> Tag7;
        internal KeyValuePair<TKey, TValue> Tag8;
        private int _tagsCount;
        private KeyValuePair<TKey, TValue>[]? _overflowTags;
        ...
        public KeyPairList(ReadOnlySpan<KeyValuePair<TKey, TValue>> tagList) : this()
        {
            ...
            switch (_tagsCount)
            {
                ...
                default:
                    ...
                    _overflowTags = new KeyValuePair<TKey, TValue>[_tagsCount + OverflowAdditionalCapacity]; // Add extra slots for more tags to add if needed
        ...
        public void Add(KeyValuePair<TKey, TValue> tag)
        {
            if (_overflowTags is not null)
            {
        ...
        public readonly void CopyTo(Span<KeyValuePair<TKey, TValue>> tags)
        {
            if (tags.Length < _tagsCount)
            {
                throw new ArgumentException("Arg_BufferTooSmall");
            }
        ...
        public readonly bool Contains(KeyValuePair<TKey, TValue> item) => IndexOf(item) >= 0;
        ...
        public readonly IEnumerator<KeyValuePair<TKey, TValue>> GetEnumerator() => new Enumerator(in this);
        ...
        /// </summary>
        /// <param name="item">The tag to locate in the <see cref="T:System.Diagnostics.TagList" />.</param>
        public readonly int IndexOf(KeyValuePair<TKey, TValue> item)
        {
            if (_overflowTags is not null)
            {
            ...
            {
                case 1:
                    if (TagsEqual(Tag1, item)) { return 0; };
                    break;
                case 2:
                    if (TagsEqual(Tag1, item)) { return 0; }
        ...
        internal readonly KeyValuePair<TKey, TValue>[]? Tags => _overflowTags;

        private static bool TagsEqual(KeyValuePair<TKey, TValue> tag1, KeyValuePair<TKey, TValue> tag2)
        {
            if (!tag1.Key.Equals(tag2.Key))
            {
                return false;
            }
        ...
        {
            _overflowTags = new KeyValuePair<TKey, TValue>[16];
        ...
        {
            private KeyPairList<TKey, TValue> _tagList;
            private int _index;
            internal Enumerator(in KeyPairList<TKey, TValue> tagList)
            {
                _index = -1;
                _tagList = tagList;
            }

            public KeyValuePair<TKey, TValue> Current => _tagList[_index];
